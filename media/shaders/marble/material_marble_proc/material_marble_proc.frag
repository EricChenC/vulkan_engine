...
void main()
{
	...
	// procedure marble
	vec3 p = outPosition * 800.0 / usp.sizeScale;
	vec3 uv = p + perlin(p)*800.;
	float t = perlin(p + perlin(uv)*800.);
	t = t * t * t;
	vec4 kd = vec4(vec3(usp.marbleColor1) * t + vec3(usp.marbleColor2) * (1.0 - t), 1.0);
