...
layout(binding = 6) uniform UniformSpecialParameters{
	...
	float particleSize;
	float micaPercentage;
	float quarPercentage;
}usp;
...
float noise(vec3 x)
{
	...
	f = f * f * (3.0 - f - f);
	float n = p.x + p.y * 57.0 + 3113.00 * p.z;

	return mix(	mix( mix(hash(n + 0.0),		hash(n + 1.0), f.x),
					 mix(hash(n + 57.0),	hash(n + 58.0), f.x),	f.y),
				mix( mix(hash(n + 3113.00),	hash(n + 3114.00), f.x),
					 mix(hash(n + 3170.0),	hash(n + 3171.0), f.x),	f.y),	f.z);
}

vec3 granite(vec3 P)
{
	float foo = noise(P * usp.particleSize);
	if (foo < usp.micaPercentage) // mica
		return vec3(usp.micaColor);
	else
	{
		foo = noise(P * usp.particleSize * 0.6);
		return (foo < usp.quarPercentage) ? vec3(usp.quarColor) : vec3(usp.feldsparColor);
	}
}
...
void main()
{
	...
	// texture lookup
	vec4 kd = vec4(granite(outPosition), 1.0);
