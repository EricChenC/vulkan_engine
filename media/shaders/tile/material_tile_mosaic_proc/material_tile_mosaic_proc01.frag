...
layout(binding = 6) uniform UniformSpecialParameters{
	...
	float mosaicShininess;
	float mosaicReflectivity;
	float mosaicDiffRough;
}usp;
...
void main()
{
	...
	//float hori = step(MWF, ppos.x) - step(1.0 - MWF, ppos.x);
    //float vert = step(MHF, ppos.y) - step(1.0 - MHF, ppos.y);
    
    vec4 kd = vec4(vec3(usp.mortarColor), 1.0);// vec4(mix(vec3(usp.mortarColor), vec3(usp.tileColor), hori * vert), 1.0);
    vec3 vBumpNml = calcNormal(ppos); // pigment normal.
    if (vBumpNml.z > 0.99) kd = vec4(vec3(usp.tileColor), 1.0);
	...
	float reflFactor = fresnelSchlick(NdotV, usp.mosaicReflectivity);
	...
	// specular reflection
	float shininess = usp.mosaicShininess;
	...
	float fSpecCoef = specularPhongAdv(vV, vL, vN, usp.mosaicShininess);
	float fDiffCoef = diffuseRough(max(dot(vN, vL), 0.0), usp.mosaicDiffRough);
