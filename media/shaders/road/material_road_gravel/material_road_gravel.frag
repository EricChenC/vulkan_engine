...
vec2 gravel(vec3 p)
{
	p = p * usp.sizeScale * 20.;
	float h = 0.5 + perlinNoise3D(p) * 0.5;
	return vec2(1.) - vec2(pow(h, 3.), h);
}
...
void main()
{
	...
	vec2 cp = gravel(outPosition);
	float g0 = cp.x;
	float gx = gravel(outPosition + vec3(esp, 0.f, 0.f)).x;
	float gy = gravel(outPosition + vec3(0.f, esp, 0.f)).x;
	float gz = gravel(outPosition + vec3(0.f, 0.f, esp)).x;
	vN = normalize(vN - vec3(gx - g0, gy - g0, gz - g0) * 100.f);
	
	if(unp.version == 2){
		float t = pow(cp.y, 8);
		kd = vec4(vec3(unp.diffuseColor) * (1.0 - t) + vec3(usp.dirtColor) * t, 1.0);
