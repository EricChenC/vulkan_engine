...
layout(binding = 7) uniform UniformSpecialTextureParameters{
	vec2 		shininessOffset;		// UV pixel offset
	vec2 		shininessRepeat;		// UV pixel repeat
	
	float		shininessScale;			// default scale
}ustp;

layout(binding = 8) uniform sampler2D shininessTexture;
...
void main()
{
	...
	//vec2 texShinCoord = outTexcoord * ustp.shininessRepeat+ ustp.shininessOffset;
	//float shininessValue = unp.shininess * luminance(texture(shininessTexture, texShinCoord).xyz) * 99.0;
	...
	// specular reflection
	float shininessValue = unp.shininess;
	...
	if (shininessValue > 0)
	{
		...
		if (shininessValue <= 1.0)			// l = 6, 5
		{
			idx = 5;
			fT = 1.0 - shininessValue;
		}
		else if (shininessValue <= 4.0)		// l = 5, 4
		{
			idx = 4;
			fT = (4.0 - shininessValue) / 3.0; // 4 - 1 = 3
		}
		else if (shininessValue <= 16.0)		// l = 4, 3
		{
			idx = 3;
			fT = (16.0 - shininessValue) / 12.0; // 16 - 4 = 12
		}
		else if (shininessValue <= 48.0)		// l = 3, 2
		{
			idx = 2;
			fT = (48.0 - shininessValue) / 32.0; // 48 - 16 = 32
		}
		else if (shininessValue <= 100.0)	// l = 2, 1
		{
			idx = 1;
			fT = max((100 - shininessValue) / 100.0, 0.0);
		}
	...
	float fSpecCoef = 0.0f;
	if(unp.version == 1){
		fSpecCoef = specularPhongAdv(vV, vL, vN, shininessValue);
	}else{
		// specular shininess texture 
		// bump texture
		vec2 texShCoord = outTexcoord * ustp.shininessRepeat + ustp.shininessOffset;
		float fShininess = luminance(texture(shininessTexture, texShCoord).xyz);
		
		fSpecCoef = specularPhongAdv(vV, vL, vN, fShininess * 100);
	}
