...
layout(binding = 6) uniform UniformSpecialParameters{
	...
	float bumpScale;
	float brushDir;				// brush direction, [0-0.5] for x, [0.5-1.4] for y and [1.5-2.4]
}usp;
...
float brushingBump(vec3 p)
{
	...
	vec3 vVerticalScale = vec3(2., 2., 2.);
	if (usp.brushDir + 0.5 < 1.0)
		vVerticalScale.x = 80.0;
	else if (usp.brushDir + 0.5 < 2.0)
		vVerticalScale.y = 80.0;
	else vVerticalScale.z = 80.0;

	float r = perlinNoise3D (p * vVerticalScale);
	return r * 0.5 + 0.5;
}
