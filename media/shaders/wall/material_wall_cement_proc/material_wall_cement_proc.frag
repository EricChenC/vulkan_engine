...
layout(binding = 6) uniform UniformSpecialParameters{
	...
	float verticalDir;				// vertical direction, [0-0.5] for x, [0.5-1.4] for y and
}usp;
...
float cement(vec3 p)
{
	...
	vec3 vVerticalScale = vec3(1., 1., 1.);
	if (usp.verticalDir + 0.5 < 1.0)
		vVerticalScale.x = 3.0;
	else if (usp.verticalDir + 0.5 < 2.0)
		vVerticalScale.y = 3.0;
	else vVerticalScale.z = 3.0;
	
	// background
	float p1 = perlinNoise3D (p * vec3(1., 3., 1.));
	p1 = truncate (0.49, 0.51, 0.5 + p1 * 0.5);
	// small pit
	float p2 = perlinNoise3D (p * 20.);
	p2 = 1. - pow(0.5 + p2 * 0.5, 30.);
