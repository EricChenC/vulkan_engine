...
// special
layout(binding = 7) uniform UniformSpecialParameters{
	vec4 lineColor;
	vec4 baseColor;
	vec4 gapColor;
...
vec4 woodProc(vec2 uv)
{
	...
	float fSpec = 0.0f;
	if(unp.version == 1){
		fSpec = min(fLine * 0.5 + 0.5, 1 - fGap);
	}else{
		fSpec = min((1. - fLine) * 0.3 + 0.7, 1 - fGap);
	}
	
	return vec4(mix(mix(vec3(usp.baseColor), vec3(usp.lineColor), fLine), vec3(usp.gapColor), fGap), fSpec);
}
...
void main()
{
	...
	vec4 kd = vec4(vec3(unp.diffuseColor) * wood.xyz, 1.0);
	vec4 ks = vec4(vec3(unp.specularColor) * wood.w, 1.0);
